...
func (h *handlerTransaction) UpdateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	var transUpdate models.Transaction
	transUpdate.ID = int64(idTrans)

	transUpdate.UserID = userId
	// if request.UserID != 0 {
	// }

	transUpdate.Total = request.Total
	// if request.Total != 0 {
	// }

	if request.Status != "" {
		transUpdate.Status = request.Status
	}

	transUpdate.CreatedAt = transaction.CreatedAt
	transUpdate.UpdatedAt = transaction.UpdatedAt
	
	fmt.Printf("Niali total : %v", transaction.Total)


	_, err = h.TransactionRepository.UpdateTransaction(transUpdate)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		response := dto.ErrorResult{Code: http.StatusInternalServerError, Message: err.Error()}
		json.NewEncoder(w).Encode(response)
	...
	// data, err := h.TransactionRepository.GetTransaction(idTrans)
	// if err != nil {
	// 	w.WriteHeader(http.StatusInternalServerError)
